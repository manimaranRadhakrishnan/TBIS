...
export class AppIdleService implements OnDestroy {
  ...
  public FINAL_LEVEL_TIMER_VALUE_IN_MIN: number=15;
  public userIdlenessChecker: BehaviorSubject<string> |any;
  public secondLevelUserIdleChecker: BehaviorSubject<string> |any;
  ...
  constructor(private zone: NgZone,private token:TokenStorageService) {
  ...
  get lastAction(): number {
    let returnUrl = localStorage.getItem('STORE_KEY') as string;
    if(returnUrl){
        return parseInt(returnUrl,10);
    }else{
  ...
  private executeFinalTimer = () => {
    AppIdleService.runSecondTimer = true;
  ...
  private initializeFinalTimer(): void {
    ...
    this.clockForIdle = interval(intervalDuration).pipe(
      ...
      takeWhile(() => AppIdleService.runSecondTimer)
    );
    ...
    this.clockForIdle.subscribe(() => {
      if (--timeInSecond === 0) {
        ...
        if (timeInMin === -1 && timeInSecond === 0) {
          AppIdleService.runSecondTimer = false;
