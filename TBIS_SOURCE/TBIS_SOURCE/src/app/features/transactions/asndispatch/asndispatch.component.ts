...
@Component({
  selector: 'app-asndispatch',
  standalone: false,
  templateUrl: './asndispatch.component.html',
  styleUrl: './asndispatch.component.css'
})

export class AsndispatchComponent implements OnInit,AfterViewInit{
  status = 'start';
  @Input() masterObj: any;
  masterData:any={};
  form: any = {};
...
overlay: boolean=false;
  ...
  showPartsAddDialog:boolean=false;
...
onSelectPart(partid:any):void{
  if(partid==null){
    ...
    console.info(this.masterData.part.spq.toString() +this.masterData.part.packingshortname.toString() )
...
if(this.enteredParts!=this.savedCardDetails?.length){
  this.toastr.info("Save the Transection before close");
}
}
...
cancelAdd(){
  if(this.enteredParts!=this.savedCardDetails?.length){
    this.toastr.info("last changes not save");
  ...
  this.service.getPartRequestById(asnId).subscribe({
    next: (rdata) => {
        if (rdata.result) {
            ...
            this.enteredParts=this.savedCardDetails?.length;

            if(this.masterData.asnStatus==2 || this.masterData.asnStatus==5 || this.masterData.asnStatus==6 || this.masterData.asnStatus==9 || this.masterData.asnStatus==8){
...
updateAsn(): void {
  ...
  if (this.IsNullorEmpty(this.masterData.deliveryNoteNo)) {
    this.toastr.info("Please enter delivery Note No");
    return;
  }
  if (this.IsNullorEmpty(this.masterData.invoiceNo)) {
    this.toastr.info("Please enter invoice No");
    return;
  }
  if (this.IsNullorEmpty(this.masterData.ewayBillNo)) {
    this.toastr.info("Please enter eway Bill No");
    return;
  }
  if (this.IsNullorEmpty(this.masterData.rgpNo)) {
    this.toastr.info("Please enter rgp No");
    return;
  }
  ...
  let data={
    ...
    partsAdd:this.masterData.partsAdd,
  }
  //Role Based
...
showPartsAddModel(){
  ...
  this.showPartsAddDialog=true;
}

closePartsAddModel(){
  this.showPartsAddDialog=false;
}
...
updateAsnPartsAdd(): void {
  let data={
    partsAdd:1,
    asnId:this.masterData.asnId
  }
  this.service.updateAsnPartsAdd(data).subscribe({
    next: (rdata) => {
        if (rdata.isSuccess) {
            // this.toastr.success(rdata.message);
            this.closePartsAddModel();
            this.updateAsn();
            this.getASNMasterById(data.asnId.toString());    
            this.action="Edit";
        } else {
            this.toastr.warning(rdata.message);
        }
