...
@Component({
  selector: 'app-asninbound',
  standalone: false,
  templateUrl: './asninbound.component.html',
  styleUrl: './asninbound.component.css'
})

export class AsninboundComponent implements OnInit,AfterViewInit{
  status = 'start';
  @Input() masterObj: any;
  masterData:any={};
  form: any = {};
...
overlay: boolean=false;
  ...
  longhaulvehicleslist:any=[];
  doActionContainerNo:string='';
  doActionInboundId:string='0';
  longhaul:any=[];
...
fileChangeAsnInboundEvent(fileInput: any) {
  ...
  if (fileInput.target!.files && fileInput.target.files[0]) {
      ...
      this.service.uploadAsnInbound(fileInput.target.files[0]).subscribe({
        next: (rdata) => {
            if (rdata.isSuccess) {
                this.toastr.success(rdata.message);
...
getAjaxVehicles(ajaxId:string):void{
  this.service.getAjaxDropDown(ajaxId).subscribe({
      next: (rdata) => {
          if (rdata.data) {
              this.longhaulvehicleslist=rdata.data;
          }
       },
      error: (e) => console.error(e),
  })
...
doAssignAction(event: any){
  this.doActionInboundId=event.inboundid;
  this.doActionContainerNo=event.containerno;
  this.longhaul.assignVehicleNo=null;
  this.openVehicleDlg();
}

vehicleAssign(vehicleInboundId:any,vehicleContainerNo:any){
  
  if(this.IsNullorEmpty(this.longhaul.assignVehicleNo) || this.longhaul.assignVehicleNo=='' || this.longhaul.assignVehicleNo==undefined){
    this.toastr.info("select vehicle");
    return;
  }

  let picked:any={};
  picked.inboundId=vehicleInboundId;
  picked.containerNo=vehicleContainerNo;
  picked.vechicleNo=this.longhaul.assignVehicleNo;
  picked.subloactionId=this.masterData.subloactionId;
  picked.loactionId=this.masterData.locationId;
  picked.warehouseId=this.masterData.warehouseId;
  picked.userId=this.WmsGV.userId;
  this.service.assignInboundVehicleUpdate(picked).subscribe({
    next: (rdata) => {
        if (rdata.isSuccess) {
            this.toastr.success(rdata.message);
