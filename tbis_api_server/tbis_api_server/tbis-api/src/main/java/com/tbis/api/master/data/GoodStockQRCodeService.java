...
public class GoodStockQRCodeService extends WritePDF {
	...
	private static final String asnPartDetail="SELECT p.partid,p.partno,p.partdescription,p.spq,p.repackqty binqty,"
			+ "p.loadingtype,p.noofpackinginpallet,c.customer_erp_code,sc.tbisscancode,"
			+ "DATE_FORMAT(sm.moveddate,'%d-%m-%Y') moveddate FROM `part_master` p  "
			+ "inner join customer_master c on c.customerId=p.customerid "
			+ "inner join stockmovementdetail sc on p.partid=sc.partid "
			+ "inner join stockmovementheader sm on sm.stockmovementid=sc.stockmovementid where sm.stockmovementid=? "
			+ "order by substring(sc.tbisscancode,41) ";
...
//				getContent().endText();
				...
				getContent().setFont(PDType1Font.TIMES_ROMAN, 10);
				getContent().newLineAtOffset(p.startX+75,p.startY-90);            
				getContent().showText(rs.getString("moveddate"));
				getContent().endText();
				if(i>0 && i%17==0) {
					closeContent();
